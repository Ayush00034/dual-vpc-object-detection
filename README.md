...
The decoupled local frontend successfully routing an image to the private backend and returning YOLOv8 bounding boxes for cars in \~176ms.
...
## Key Technical Learnings
  ...
  * **Cross-VPC Routing:** Discovered that ALBs cannot target "Instance IDs" across different VPCs; it requires explicitly routing to the private IP address instead.
  ...
  * **Production WSGI:** Transitioned from a standard Flask development server to a production-ready NGINX + Gunicorn stack, managed via Systemd for automatic background restarts.